...
describe('Blog app', () => {
  ...
  describe('When logged in', () => {
    ...
    test('only author sees delete button', async ({ page }) => {
      ...
      for (const blog of await page.getByTestId('aBlogPost').all()) {
        ...
        if (author === 'aAuthor') {
          //the author that we are logged in as
          await expect(
    ...
    test('Blogs arranged in order of likes', async ({ page }) => {
      ...
      for (const blog of await page.getByTestId('aBlogPost').all()) {
        ...
        const numLikes = parseInt(
          (await blog.getByTestId('postLikes').textContent()).slice(8),
        )
